    ...
    if(validData)
    {
        ...
        $.post("/ajax/createEvent", postPackage, function (data){
			...
			else
			{
                ...
                $('#event-data-holder').html("<p class = 'eventServerResponse'> Your event has been created! Here is the exclusive url for your event: </p><br><p class='eventServerResponseURL'><a href='"+window.location.protocol+"//"+window.location.host+data.responseStr+"'>"+window.location.protocol+"://"+window.location.host+data.responseStr+"</a></p>");
			}
        });
    }
    return false;
